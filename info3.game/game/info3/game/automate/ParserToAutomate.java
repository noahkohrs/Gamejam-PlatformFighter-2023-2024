...
public class ParserToAutomate implements IVisitor {
    ...
    public Object visit(BinaryOp operator, Object left, Object right) {
        //System.out.println("8 "+left.toString()+" "+operator.toString()+" "+right.toString());
        return null;
    }
    ...
    public Object visit(State state) {
        ...
        info3.game.automate.State st=new info3.game.automate.State(statename);
        if(autos.get(autos.size()-1).src==null)
            autos.get(autos.size()-1).src=st;
        autos.get(autos.size()-1).addState(st); 
        if(autos.get(autos.size()-1).trans==null)
            return null;
        Transitions tr=autos.get(autos.size()-1).trans.get(autos.get(autos.size()-1).trans.size()-1);
        if(tr.src==null){
            tr.src=st;
            return null;}
        if(tr.dest==null)
    ...
    public void enter(Condition condition) {
        ...
        try {
            if(((FunCall)condition.expression).parameters.size()==0)
                ...
                condInstance = condClass.getDeclaredConstructor(String.class).newInstance(((FunCall)condition.expression).parameters.get(0).toString());
        ...
        if( autos.get(autos.size()-1).trans.get(autos.get(autos.size()-1).trans.size()-1).cond!=null){
            ...
            a.trans.add(a.trans.size(), new Transitions(autos.get(autos.size()-1).src, null, null, null));
        }
        autos.get(autos.size()-1).trans.get(autos.get(autos.size()-1).trans.size()-1).cond=(info3.game.automate.condition.Condition) condInstance;
    }
    ...
    public void enter(Action action) {
        ...
        try {
            if(action.calls.get(0).parameters.size()==0)
                ...
                actionInstance = actionClass.getDeclaredConstructor(String.class,String.class).newInstance(param1,param2);
            }
        } catch (InstantiationException | IllegalAccessException | IllegalArgumentException | InvocationTargetException
                | NoSuchMethodException | SecurityException e) {
        ...
        autos.get(autos.size()-1).trans.get(autos.get(autos.size()-1).trans.size()-1).action=(info3.game.automate.action.Action) actionInstance;
    }
    ...
    public void enter(Automaton automaton) {
        ...
        Automate auto=new Automate(null, (Entity) entityInstance, null, null);
